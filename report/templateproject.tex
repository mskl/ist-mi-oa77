...
The only two possible outcomes that might happen are the following:
\begin{itemize}
	\item The robot passes within the disc
		$$\text{if } d(p(\tau_{k}), c_{k}) \leq r_{k} \text{ then } d_{k} = 0$$
	\item The robot passes outside of the disc
		$$\text{if } d(p(\tau_{k}), c_{k}) > r_{k}\text{ then }d_{k} = d(p(\tau_{k}), c_{k}) - r_{k}$$
\end{itemize}
The whole equation can be written as
$$\textit{max}(0, d(p(\tau_{k}), c_{k}) - r_{k})$$
